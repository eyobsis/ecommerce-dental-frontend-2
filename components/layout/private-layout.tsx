...
const adminModules: MenuModule[] = [
  { key: "dashboard", icon: LayoutDashboard, title: "Dashboard", url: "/admin/dashboard" },
  { key: "orders", icon: ShoppingCart, title: "Orders", url: "/admin/orders" },
  { key: "products", icon: Package, title: "Products", url: "/admin/products" },
  { key: "categories", icon: Tags, title: "Categories", url: "/admin/categories" },
...
const roleModuleAccess: Record<Role, string[]> = {
  ...
  ADMIN: ["dashboard", "orders", "products", "categories", "customers", "inventory", "reports", "users", "settings"],
  OWNER: ["dashboard", "orders", "products", "categories", "customers", "inventory", "reports", "users"],
  MANAGER: ["dashboard", "orders", "products", "customers", "inventory", "reports"],
...
const PrivateLayout: React.FC<{ roles?: Role[]; children: ReactNode }> = ({
  ...
  const allowedKeys = Array.from(new Set(roles.flatMap((r) => roleModuleAccess[r] || [])));
  ...
  const activeKey = pathToKey[pathname] || (isClientOnly ? "my-orders" : "dashboard");
  ...
  return (
    <SidebarProvider>
      {/* 1. APP SIDEBAR */}
      <Sidebar variant="inset" collapsible="icon" className="border-r shadow-sm font-poppins">
          ...
          <SidebarGroup>
            ...
            <SidebarMenu>
              {visibleModules.map((module, index) => {
                ...
                if (hasItems) {
                  return (
                    <Collapsible key={module.key} asChild defaultOpen={index === 0} className="group/collapsible">
                      <SidebarMenuItem>
                ...
                return (
                  <SidebarMenuItem key={module.key}>
                    <SidebarMenuButton asChild isActive={isActive} tooltip={module.title}>
              ...
              <DropdownMenuContent className="w-56 font-poppins" align="end" forceMount>
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground leading-none">admin@royalimport.com</p>
                  </div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuGroup>
